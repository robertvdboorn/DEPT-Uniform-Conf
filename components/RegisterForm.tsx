            ...
            {registered ? (
              <h1
                ...
                dangerouslySetInnerHTML={{
                  __html: props?.contentfulEntry?.success,
                }}
              />
            ) : (
              <h1
                className="my-4 text-5xl font-bold leading-tight"
                dangerouslySetInnerHTML={{ __html: props?.contentfulEntry?.heading }}
              />
            )}
            <form>
              {registered ? (
                <>
                  <p className="pb-16">{props?.contentfulEntry?.registeredText}</p>
                  <Link legacyBehavior href="/">
                    <a className="mx-auto lg:mx-0 hover:underline bg-white text-gray-800 font-bold rounded-full my-6 py-4 px-8 shadow-lg">
                      {props?.contentfulEntry?.homeLinkText}
                    </a>
                  </Link>
                </>
              ) : (
